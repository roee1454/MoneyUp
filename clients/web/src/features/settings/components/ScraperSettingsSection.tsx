...
export function ScraperSettingsSection({ userProfile }: ScraperSettingsSectionProps) {
  ...
  const handleAutoDetectChromium = async () => {
    ...
    try {
      ...
      if (result.data?.availableBrowsers) setAvailableBrowsers(result.data.availableBrowsers);
      const detectedPath = result.data?.success && result.data.path ? result.data.path : null;
      ...
      saveScraperSettings.mutate({
        ...
        scraperAutoSyncCooldownSeconds: userProfile?.scraperAutoSyncCooldownSeconds ?? 1800,
    ...
    <div className="space-y-6">
      ...
      {isDetecting ? (
        <PremiumCard className="p-12 flex flex-col items-center justify-center text-center space-y-4 border-border/60">
          ...
          <div className="space-y-1">
            <p className="text-sm font-black text-foreground">מחפש דפדפן תואם במערכת...</p>
            <p className="text-[11px] font-medium text-muted-foreground">פעולה זו מתבצעת באופן אוטומטי לצורך הגדרת הסורקים</p>
          </div>
        </PremiumCard>
      ) : isInstalling ? (
        <PremiumCard className="p-8 flex flex-col space-y-6 border-primary/30 bg-primary/5 relative overflow-hidden">
          <div className="flex flex-col items-center text-center space-y-4">
            <CircleNotch className="h-10 w-10 animate-spin text-primary" />
            <div className="space-y-1">
              <p className="text-sm font-black text-foreground">מתקין Chromium במערכת...</p>
              <p className="text-[11px] font-medium text-muted-foreground leading-relaxed">פעולה זו עשויה לקחת מספר דקות, אנא אל תסגור את הדף.</p>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex justify-between items-end">
              <span className="text-[10px] font-black text-primary uppercase">התקדמות ההורדה</span>
              <span className="text-xs font-mono font-bold text-primary">{installProgress}%</span>
            </div>
          ...
          <div className="bg-zinc-950 p-4 font-mono text-[10px] text-zinc-300 space-y-1 h-32 overflow-y-auto border border-white/5">
            {installLogs.length === 0 ? <p className="text-zinc-600 italic">ממתין לתחילת התהליך...</p> : installLogs.map((log, i) => (
              <p key={i} className="break-all opacity-80"><span className="text-primary mr-2">›</span>{log}</p>
            ))}
          </div>
        </PremiumCard>
        ...
        <div className="space-y-4">
          <PremiumCard className="p-8 flex flex-col items-center justify-center text-center space-y-5 border-destructive/30 bg-destructive/5">
            ...
            <div className="space-y-2">
              <p className="text-base font-black text-foreground">לא הוגדר דפדפן לסריקה אוטומטית</p>
              <p className="text-xs font-medium text-muted-foreground leading-relaxed">כדי לאפשר סנכרון אוטומטי מול הבנקים, מומלץ להתקין את רכיב הדפדפן הפנימי (Chromium).</p>
            </div>
            ...
            <div className="flex flex-col gap-3 w-full">
              <Button onClick={handleInstallChromium} disabled={isInstalling} className="h-11 px-6 text-xs font-black bg-primary text-primary-foreground hover:bg-primary/90 rounded-none w-full shadow-xl shadow-primary/10">
                <DownloadSimple weight="bold" className="ml-2 h-4 w-4" />
                <span>התקן Chromium באופן אוטומטי</span>
              </Button>
              <Button onClick={handleAutoDetectChromium} variant="outline" className="h-11 px-6 text-xs font-black border-border hover:bg-accent rounded-none w-full">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between group/item">
              <div className="space-y-0.5">
                <div className="flex items-center gap-2">
                  ...
                  <label className="text-sm font-black text-foreground">הדפדפן מוגדר ומוכן</label>
                </div>
                <p className="text-[11px] font-medium text-muted-foreground">הסורק האוטומטי מוכן לפעולה</p>
              </div>
              <button onClick={() => setShowAdvancedScraper(!showAdvancedScraper)} className="text-[10px] font-black text-muted-foreground hover:text-primary transition-colors">
              ...
              <div className="space-y-4 pt-2 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <label className="text-sm font-black text-foreground">נתיב דפדפן (Chromium)</label>
                    </div>
                    <button onClick={handleAutoDetectChromium} className="text-[10px] font-black text-primary hover:underline flex items-center gap-1 cursor-pointer">
                      <MagnifyingGlass weight="bold" className="h-3 w-3" />
                      <span>סריקה מחדש</span>
                    </button>
                  </div>
                  <div onClick={() => {
                    const path = window.prompt('עדכן את נתיב הדפדפן:', scraperChromiumPath);
                ...
                <div className="flex items-center justify-between group/item">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <Browser className="h-4 w-4 text-muted-foreground" />
                      <label className="text-sm font-black text-foreground">הצגת דפדפן</label>
                    </div>
                    <p className="text-[11px] font-medium text-muted-foreground">הצגת תהליך הסריקה בחלון נפרד</p>
                  </div>
                  <Switch checked={scraperShowBrowser} onCheckedChange={setScraperShowBrowser} />
                </div>
              </div>
            )}
            <div className="h-px bg-border w-full" />
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <label className="text-sm font-black text-foreground">ניסיונות סריקה חוזרים</label>
              </div>
              <Select value={String(scraperTimeoutRetryCount)} onValueChange={(val) => setScraperTimeoutRetryCount(Number(val))}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <label className="text-[11px] font-black text-foreground leading-tight">זמן התחברות (שניות)</label>
                </div>
                <input type="number" value={scraperLoginTimeoutSeconds} onChange={(e) => setScraperLoginTimeoutSeconds(Number(e.target.value))} className="h-10 w-full border border-border bg-muted/30 px-3 text-xs font-bold focus:bg-card focus:outline-none transition-all rounded-none text-foreground" />
              </div>
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <Clock className="h-4 w-4 text-muted-foreground" />
                  <label className="text-[11px] font-black text-foreground leading-tight">זמן סריקה (שניות)</label>
                </div>
                <input type="number" value={scraperDefaultTimeoutSeconds} onChange={(e) => setScraperDefaultTimeoutSeconds(Number(e.target.value))} className="h-10 w-full border border-border bg-muted/30 px-3 text-xs font-bold focus:bg-card focus:outline-none transition-all rounded-none text-foreground" />
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <ArrowsClockwise className="h-4 w-4 text-muted-foreground" />
                <label className="text-sm font-black text-foreground">זמן המתנה בין סנכרונים</label>
              </div>
              <div className="flex gap-2">
                <input type="number" value={cooldownValue} onChange={(e) => setCooldownValue(Number(e.target.value))} className="h-10 w-24 border border-border bg-muted/30 px-3 text-xs font-bold focus:bg-card focus:outline-none transition-all rounded-none text-foreground" />
                <Select value={cooldownUnit} onValueChange={(val: any) => setCooldownUnit(val)} className="flex-1">
            ...
            <Button onClick={handleSaveScraperSettings} disabled={saveScraperSettings.isPending} className="w-full h-11 bg-primary hover:bg-primary/90 text-primary-foreground rounded-none font-black text-xs transition-all flex items-center justify-center gap-2 shadow-xl shadow-primary/10">
              {saveScraperSettings.isPending ? <CircleNotch className="h-4 w-4 animate-spin" /> : <span>שמירת כל ההגדרות</span>}
            </Button>
          </div>
        </PremiumCard>
      )}
      ...
      <PremiumCard className="p-5 bg-muted/10 border-dashed border-border">
        <div className="flex gap-4">
          <div className="h-10 w-10 shrink-0 rounded-full bg-muted flex items-center justify-center"><Warning className="h-5 w-5 text-muted-foreground" /></div>
          <div className="space-y-1">
            <h4 className="text-sm font-black text-foreground leading-tight">צריך עזרה?</h4>
            <p className="text-[11px] font-medium text-muted-foreground leading-relaxed">הגדרות אלו משפיעות ישירות על יציבות הסנכרון מול הבנקים. במקרה של תקלות חוזרות, מומלץ להעלות את זמני ה-Timeout או להפעיל את תצוגת הדפדפן כדי לאבחן את הבעיה.</p>
          </div>
        </div>
      </PremiumCard>
    </div>
