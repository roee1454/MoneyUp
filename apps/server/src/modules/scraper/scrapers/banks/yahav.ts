...
/**
 * Class representing YahavScraper.
 */
@Injectable()
export class YahavScraper extends BaseScraper {
  constructor(
  ...
  readonly companyId = CompanyTypes.yahav;
  ...
  protected async simulateScrape(
    ...
    try {
      ...
      return {
        status: 'FAILED',
        error: err?.message || 'Unexpected Yahav scraper crash occurred',
      };
    }
  }
  ...
  protected async liveScrape(
    ...
    try {
      ...
      if (!scrapeResult.success) {
        ...
        return {
          ...
          error:
            errorParts.length > 0
              ? errorParts.join(': ')
              : 'Unknown error occurred during Yahav scraping',
        };
      }
      ...
      return {
        status: 'FAILED',
        error: (err as Error)?.message || 'Unexpected Yahav scraper crash occurred',
      };
    }
  }
}
