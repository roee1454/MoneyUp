...
export class CacheService {
  ...
  async getLatestTransaction(userId: string): Promise<TransactionEntity | null> {
  ...
  async setCachedAccounts(
    ...
    if (cacheEntry.cachedData) {
      try {
        existingAccounts = JSON.parse(cacheEntry.cachedData);
      } catch {
        existingAccounts = [];
      }
    }

    const newAccountsMetadata = accounts.map((acc) => ({
      accountNumber: acc.accountNumber,
      balance: acc.balance,
      transactions: [],
    }));

    const newAccountNumbers = new Set(newAccountsMetadata.map((a) => a.accountNumber));
    const mergedAccounts = [
      ...existingAccounts.filter((a) => !newAccountNumbers.has(a.accountNumber)),
      ...newAccountsMetadata,
    ];

    cacheEntry.cachedData = JSON.stringify(mergedAccounts);
