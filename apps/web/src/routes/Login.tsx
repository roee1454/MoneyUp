...
import type { ProfileCreationInput } from '@money-up/types';
...
export default function Login() {
  ...
  async function createProfile(data: ProfileCreationInput) {
    setError('');
    ...
    try {
      await createUserMutation.mutateAsync({
        ...
        unlockKey: data.lockProfile ? data.unlockKey : undefined,
      });
      setShowForm(false);
    } catch (err: any) {
  ...
  async function unlockAndLogin(unlockKey: string) {
  ...
  async function deleteProfile(confirmationUserId: string) {
  ...
  return (
    ...
    >
      <div className="relative z-10 w-full max-w-5xl py-8">
        ...
        <AnimatePresence mode="wait">
          {usersQuery.isLoading ? (
            ...
            >
              <CreateProfileForm
                onSave={(data) => void createProfile(data)}
      ...
      <UnlockProfileDialog
        ...
        onUnlock={(key) => void unlockAndLogin(key)}
      ...
      <DeleteProfileDialog
        ...
        onDelete={(conf) => void deleteProfile(conf)}
