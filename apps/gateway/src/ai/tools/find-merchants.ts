    ...
    try {
      ...
      const annotations = await firstValueFrom(
        this.scraperServiceClient
          .send<any[]>('spending_get_all_annotations', {})
          .pipe(timeout(15000)),
      ).catch(() => [] as any[]);
...
Return ONLY a valid JSON array of strings. Do not include markdown code block styling or any explanations outside the JSON.`;
          ...
          const aiResponse = await firstValueFrom(
            this.aiServiceClient.send<any>('ai_prompt', {
      ...
      const fallbackMerchants = await firstValueFrom(
        this.scraperServiceClient
          .send<string[]>('spending_find_merchants', {
            topic: args.topic,
          })
          .pipe(timeout(10000)),
      ).catch(() => [] as string[]);
