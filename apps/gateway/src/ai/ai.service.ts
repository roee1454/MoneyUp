...
import { firstValueFrom, Observable, Subscriber, timeout } from 'rxjs';
...
export class GatewayAiService {
      ...
      if (response.type === 'tool_calls') {
        ...
        if (conversationId) {
          await firstValueFrom(
            this.usersServiceClient.send('user_add_message', {
              userId,
              conversationId,
              ...assistantMsg,
            }),
  ...
  async runStreamLoop(
    ...
    const response$ = this.aiServiceClient.send<any>('ai_prompt_stream', {
      ...resolvedPayload,
      tools: AI_TOOLS,
    });
